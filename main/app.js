...
let receipts = [];
let storageKey = "receipt-moa-guest";
...
function saveLocal() {
  if (currentUser) localStorage.setItem(storageKey, JSON.stringify(receipts));
}
...
document.querySelector("#logoutBtn").addEventListener("click", async () => {
  try { await authPersistenceReady; await signOut(auth); } finally { window.location.replace("../login/"); }
});
...
document.querySelector("#clearDemoBtn").addEventListener("click", () => { receipts = []; saveLocal(); render(); });
